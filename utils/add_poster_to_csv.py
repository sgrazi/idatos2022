...
def query_id(movieid):
    try:
        ...
        movie_page = requests.get('http://www.imdb.com/title/'+movieid,headers=headers)
        soup = BeautifulSoup(movie_page.text, 'html.parser')
        mediaviewer = soup.find('a', {'class': 'ipc-lockup-overlay ipc-focusable'}).get('href')
        poster_page = requests.get('https://www.imdb.com' + mediaviewer,headers=headers)
